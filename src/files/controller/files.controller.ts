...
export class FilesController {
  ...
  /**
   * Renders the actual file document in pdf, image or the actual file format with the restrictedKey of the Access Control in Request path parameter
  ...
  @Get('/view/:key')
  @ApiOperation({
    summary: 'View/Render File Document',
    description:
      'Renders the actual file document in pdf, image or the actual file format with the restrictedKey of the Access Control in Request path parameter',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Fetches the file by the fileId and returns its details. Hides the private s3 stored key of the file from the response.',
  })
  ...
  /**
   * Searches files based on metadata filters. This metadata filter is based on the properties present in the metadata of the file like studentName, Course etc. Metadata is dynamic, schemaless.
  ...
  /**
   * Fetches all files that a user owns. The files which were uploaded by user or issued by someone appear in this.
  ...
  @Delete()
  @ApiOperation({
    summary: 'Delete/Recover File',
    description:
      'Deletes/recovers a file by its fileId. This api can also permanently delete the file from the database if action is PERMANENT_DELETE',
  })
